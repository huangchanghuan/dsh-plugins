...
const readme = await readFile(join(root, 'docs/PLUGINS.md'), 'utf8')
const englishReadme = await readFile(join(root, 'docs/PLUGINS.en.md'), 'utf8')
...
for (const plugin of catalog.plugins) {
  ...
  if (!englishReadme.includes(englishDescription)) {
    fail(`PLUGINS.en 文档缺少插件介绍：${plugin.repository}`)
  }
}

if (!readme.includes('[English](README.en.md)')) fail('PLUGINS 文档缺少英文版入口')
if (!englishReadme.includes('[简体中文](README.md)')) fail('PLUGINS.en 文档缺少中文版入口')
