...
async function githubFetch(path, { optional = false, raw = false } = {}) {
  const headers = {
    ...
    'User-Agent': 'huangchanghuan-dsh-plugins',
...
if (process.argv.includes('--readme-only')) {
  ...
  await writeFile(join(root, 'docs/PLUGINS.md'), generateChineseReadme(catalog.generatedAt, catalog.plugins))
  await writeFile(join(root, 'docs/PLUGINS.en.md'), generateEnglishReadme(catalog.generatedAt, catalog.plugins))
...
await writeFile(join(root, 'docs/PLUGINS.md'), generateChineseReadme(snapshot, plugins))
await writeFile(join(root, 'docs/PLUGINS.en.md'), generateEnglishReadme(snapshot, plugins))
