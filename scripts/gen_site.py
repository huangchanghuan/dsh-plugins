# -*- coding: utf-8 -*-
"""Generate the DeepSeek Whale bilingual DSH plugin directory site.

Pages:
- index.html (zh) + en.html (en)          — searchable directory home
- plugins/<owner>/<repo>.html             — per-plugin detail page (zh)
- en/plugins/<owner>/<repo>.html          — per-plugin detail page (en)
- about.html / privacy.html + en variants — content pages
- sitemap.xml / feed.xml / robots.txt / manifest.webmanifest / favicon.svg
- llms.txt (with BOM) + llms-full.txt     — LLM-facing site summaries

All internal links use root-absolute `.html` URLs (the WorkBuddy static host
does not rewrite clean URLs). No references to any third-party aggregator.
"""
...
def detail_rel(p, lang):
    """Root-absolute path of the detail page for plugin p in lang."""
    base = "en/plugins/" if lang == "en" else "plugins/"
    return base + esc(p["o"].lower()) + "/" + esc(p["n"].lower()) + ".html"

def detail_url(p, lang):
    return SITE + detail_rel(p, lang)
...
"canonical":SITE,
# detail-page strings
"d_title":f"DSH Plugins 插件详情 · {total} 个 DeepSeek Harness 插件目录",
"d_home":"目录首页","d_plugins":"插件列表","d_install_h":"安装命令","d_copy_hint":"点击「复制」后在 DeepSeek Harness 终端执行",
"d_view_source":"在 GitHub 查看源码 ↗","d_upstream":"上游仓库","d_version":"版本","d_updated":"最近更新",
...
"收录流程：数据机器人每天自动发现带 dsh-plugin 主题的公开仓库，校验其根目录 package.json 声明的 dsh.bundle.patch 补丁文件真实存在、仓库未归档且不是模板，通过后才进入目录。",
"收录不代表官方背书、安全审计或运行兼容。插件地址与介绍来自各插件的上游仓库，插件版权与许可证归各上游项目所有。安装前请自行阅读上游源码、权限与许可证。",
"数据每天自动更新一次；机器可读目录见 catalog/plugins.json，站点说明见 llms.txt 与 llms-full.txt。"],
"privacy_h2":"隐私说明",
"privacy_body":["本站是纯静态页面，不使用任何分析、统计或广告跟踪服务，不在服务器端记录你的浏览行为。",
"搜索与筛选参数仅保存在你的浏览器地址栏与 localStorage（主题偏好、视图偏好），不会上传到任何服务器。",
"点击插件链接会离开本站前往 GitHub 等外部服务，这些服务按其自身政策处理数据。",
"复制安装命令功能在你的浏览器本地完成，不产生网络请求。"],
"canonical":SITE,
},
"en": {
...
# detail-page strings
"d_title":f"DSH Plugins Plugin Details · {total} DeepSeek Harness Plugins",
"d_home":"Directory home","d_plugins":"All plugins","d_install_h":"Install command","d_copy_hint":"Click \"Copy\", then run it in the DeepSeek Harness terminal",
"d_view_source":"View source on GitHub ↗","d_upstream":"Upstream repository","d_version":"Version","d_updated":"Last updated",
...
"Listing process: a data bot discovers public repositories carrying the dsh-plugin topic every day and verifies that the dsh.bundle.patch declared in each repository's root package.json really exists, that the repository is not archived, and that it is not a template. Only then is the plugin listed.",
"Inclusion does not imply official endorsement, a security audit, or runtime compatibility. Links and descriptions come from each plugin's upstream repository; plugin copyright and licenses remain with the upstream projects. Review upstream source, permissions and license before installing.",
"The catalog refreshes daily. Machine-readable data: catalog/plugins.json. Site notes for LLMs: llms.txt and llms-full.txt."],
"privacy_h2":"Privacy notice",
"privacy_body":["This site is fully static. It uses no analytics, telemetry or advertising services, and does not record your browsing on the server side.",
"Search and filter parameters stay in your browser's address bar and localStorage (theme and view preferences) and are never uploaded.",
"Plugin links leave this site for GitHub and other external services, which process data under their own policies.",
"The copy-install-command feature runs entirely in your browser and performs no network requests."],
"canonical":SITE+"en.html",
}}
...
def cards_html(L):
  ...
  <div class="chead"><span class="rank">#{i}</span><h3><a href="/{detail_rel(p, "zh" if L is S["zh"] else "en")}">{esc(p["n"])}</a></h3><span class="stars">★ {stars_fmt(p["s"])}</span></div>
  <p class="meta"><a class="gh" href="https://github.com/{esc(p["r"])}" target="_blank" rel="noopener">@{esc(p["o"])}</a> · {esc(p["l"])} · {esc(p["li"])}{ver}</p>
  <p class="desc">{desc}</p>
...
def ld_json(L):
    lang_key = "en" if L is S["en"] else "zh"
    ld = {
        ...
        {"@type": "CollectionPage", "url": L["canonical"], "name": L["og_title"],
         ...
         "mainEntity": {"@type": "ItemList", "numberOfItems": total,
            "itemListElement": [{"@type":"ListItem","position":i,"name":p["r"],
                "url":detail_url(p, lang_key)} for i,p in enumerate(plugins[:24],1)]}},
...
.meta a.gh{color:var(--tx2);text-decoration:underline dotted}
...
<header>
  <div class="nav">
    ...
    <nav class="links">
      <a href="__HOME__#plugins">__NAV_CATALOG__</a>
      <a href="__HOME__#guide">__NAV_GUIDE__</a>
      <a href="__HOME__#faq">__NAV_FAQ__</a>
...
function cardHTML(p,i){
  ...
  return '<li class="card"><div class="chead"><span class="rank">#'+(i+1)+'</span><h3><a href="/'+esc(p.dp)+'">'+esc(p.n)+'</a></h3><span class="stars">★ '+fmt(p.s)+'</span></div>'
    +'<p class="meta"><a class="gh" href="https://github.com/'+esc(p.r)+'" target="_blank" rel="noopener">@'+esc(p.o)+'</a> · '+esc(p.l)+' · '+esc(p.li)+ver+'</p>'
    +'<p class="desc">'+d+'</p>'
...
def build(lang_key, other_page, other_hreflang):
    ...
    reps = {
        ...
        "__CARDS__": cards_html(L), "__DATA__": data_json_with_dp(lang_key),
...
def data_with_dp(lang_key):
    out = []
    for p in plugins:
        q = dict(p)
        q["dp"] = detail_rel(p, lang_key).lstrip("/")
        out.append(q)
    return json.dumps(out, ensure_ascii=False, separators=(",", ":")).replace("</", "<\\/")

def data_json_with_dp(lang_key):
    return data_with_dp(lang_key)
...
nav.links a{color:var(--tx2);font-size:14px}nav.links a:hover{color:var(--ac)}
nav.links a.lang{border:1px solid var(--card-br);border-radius:999px;padding:3px 12px}
#themeBtn{border:1px solid var(--card-br);background:var(--card);color:var(--tx);border-radius:10px;padding:6px 12px;cursor:pointer;font-size:14px}
.crumbs{max-width:860px;margin:0 auto;padding:22px 24px 0;color:var(--tx2);font-size:13px;display:flex;gap:8px;flex-wrap:wrap;align-items:center}
.crumbs a{color:var(--tx2)} .crumbs a:hover{color:var(--ac)}
.wrap{max-width:860px;margin:0 auto;padding:16px 24px 40px}
.panel{background:var(--card);border:1px solid var(--card-br);border-radius:18px;padding:28px}
.dhead{display:flex;align-items:baseline;gap:14px;flex-wrap:wrap}
.dhead h1{font-size:clamp(22px,4vw,32px);font-weight:900;letter-spacing:-.3px;word-break:break-all}
.dhead .stars{color:var(--star);font-size:15px;font-weight:700}
...
.cmd{display:flex;align-items:center;gap:10px;background:var(--code);border:1px solid var(--card-br);border-radius:12px;padding:12px 16px}
.cmd code{flex:1;font-family:ui-monospace,Consolas,monospace;font-size:13px;color:var(--ac);word-break:break-all}
.copy{border:none;background:var(--ac);color:#04222c;font-weight:700;border-radius:8px;padding:6px 14px;cursor:pointer;font-size:13px;white-space:nowrap}
.ghbtn{display:inline-block;margin-top:10px;border:1px solid var(--ac);color:var(--ac);border-radius:10px;padding:8px 16px;font-size:14px;font-weight:700}
.ghbtn:hover{background:var(--ac);color:#04222c;text-decoration:none}
.note{margin-top:18px;font-size:12.5px;color:var(--tx2)}
.actions{margin-top:22px}
.actions a.back{color:var(--ac2);font-size:14px}
#copyStatus{position:fixed;left:-9999px}
footer{border-top:1px solid var(--card-br);margin-top:36px}
footer .in{max-width:1200px;margin:0 auto;padding:26px 24px;color:var(--tx2);font-size:13px;display:flex;gap:16px;flex-wrap:wrap;justify-content:space-between}
</style>
...
<main>
<div class="crumbs"><a href="__HOME__">__D_HOME__</a><span>›</span><a href="__HOME____ANCHOR__">__D_PLUGINS__</a><span>›</span><span>__REPO__</span></div>
<div class="wrap">
  <div class="panel">
    ...
    <div class="chips">
      ...
      <span class="chip">__D_UPDATED__ <b>__UPDATED__</b></span>
    </div>
    <div class="dsection"><h2>__D_DESC_H__</h2><p class="desc">__DESC__</p></div>
    <div class="dsection">
...
<footer>
  <div class="in"><div>__FOOTER_L__</div><div>__FOOTER_R__</div></div>
</footer>
<p id="copyStatus" role="status" aria-live="polite"></p>
<script>
const MSG = {copy:"__COPY__",copied:"__COPIED__"};
const $ = id => document.getElementById(id);
document.addEventListener('click',e=>{
  const b=e.target.closest('.copy');if(!b)return;
  const done=()=>{b.textContent=MSG.copied;$('copyStatus').textContent=b.dataset.cmd+' ✓';
...
def render_detail(p, lang_key):
    L = S[lang_key]
    other = "en" if lang_key == "zh" else "zh"
    desc = esc(p["d"]) if p["d"] else L["d_none"]
    ver = esc(p["v"]) if p["v"] else L["d_unknown"]
    upd = esc(p["u"]) if p["u"] else L["d_unknown"]
    lic = L["d_unknown"] if p["li"] == "NOASSERTION" else esc(p["li"])
    title = f'{esc(p["n"])} · DSH 插件详情' if lang_key == "zh" else f'{esc(p["n"])} · DSH Plugin Details'
    short = (esc(p["d"])[:150] + "…") if len(p["d"]) > 150 else (esc(p["d"]) or esc(L["d_none"]))
    meta_desc = short + ("｜DSH 插件详情与安装命令。" if lang_key == "zh" else " | DSH plugin details and install command.")
    canonical = detail_url(p, lang_key)
    alt_zh = detail_url(p, "zh")
    ...
    ld = {
        "@context": "https://schema.org",
        "@graph": [
            {"@type": "SoftwareSourceCode", "name": p["n"], "url": canonical,
             "codeRepository": f"https://github.com/{p['r']}", "description": p["d"] or "",
             "programmingLanguage": p["l"], "license": f"https://spdx.org/licenses/{p['li']}.html" if p["li"] not in ("NOASSERTION",) else None,
             "author": {"@type": "Person", "name": p["o"]}, "inLanguage": L["html_lang"]},
            {"@type": "BreadcrumbList", "itemListElement": [
                {"@type": "ListItem", "position": 1, "name": L["d_home"], "item": SITE if lang_key == "zh" else SITE + "en.html"},
                {"@type": "ListItem", "position": 2, "name": p["r"], "item": canonical}]},
        ],
    }
    ...
    reps = {
        ...
        "__LD__": ld_json_s,
        "__HOME__": home, "__ANCHOR__": anchor,
        "__LANG_HREF__": "/" + detail_rel(p, other), "__LANG_HREFLANG__": "zh-CN" if other == "zh" else "en",
        "__NAV_LANG__": S[other]["nav_lang"] if False else ("中文" if lang_key == "en" else "English"),
        "__BTN_LIGHT__": L["btn_to_light"], "__BTN_DARK__": L["btn_to_dark"],
        "__D_HOME__": L["d_home"], "__D_PLUGINS__": L["d_plugins"],
        "__D_AUTHOR__": L["d_author"], "__D_LANGUAGE__": L["d_language"],
        "__D_LICENSE__": L["d_license"], "__D_VERSION__": L["d_version"],
        "__D_UPDATED__": L["d_updated"], "__D_DESC_H__": L["d_desc_h"],
        "__D_INSTALL_H__": L["d_install_h"], "__D_COPY_HINT__": L["d_copy_hint"], "__D_NOTE__": L["d_note"],
        "__D_VIEW_SOURCE__": L["d_view_source"], "__D_BACK__": L["d_back"],
        "__REPO__": esc(p["r"]), "__OWNER__": esc(p["o"]), "__NAME__": esc(p["n"]),
...
[data-theme="light"]{color-scheme:light;--bg1:#eef5fa;--bg2:#dcebf5;--card:#fff;--card-br:#d5e3ee;--tx:#12293b;--tx2:#54728a;--ac:#0891b2;--ac2:#0284c7;--star:#d97706;--code:rgba(8,145,178,.07)}
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC","Microsoft YaHei",sans-serif;background:linear-gradient(160deg,var(--bg1),var(--bg2));color:var(--tx);line-height:1.8;min-height:100vh}
a{color:var(--ac2);text-decoration:none}a:hover{text-decoration:underline}
header{position:sticky;top:0;z-index:50;backdrop-filter:blur(14px);background:rgba(7,21,34,.72);border-bottom:1px solid var(--card-br)}
[data-theme="light"] header{background:rgba(238,245,250,.8)}
.nav{max-width:1200px;margin:0 auto;display:flex;align-items:center;gap:18px;padding:14px 24px;flex-wrap:wrap}
...
.logo span{background:linear-gradient(90deg,var(--ac),var(--ac2));-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent}
nav.links{display:flex;gap:16px;margin-left:auto;flex-wrap:wrap;align-items:center}
nav.links a{color:var(--tx2);font-size:14px}
nav.links a.lang{border:1px solid var(--card-br);border-radius:999px;padding:3px 12px}
#themeBtn{border:1px solid var(--card-br);background:var(--card);color:var(--tx);border-radius:10px;padding:6px 12px;cursor:pointer;font-size:14px}
.wrap{max-width:860px;margin:0 auto;padding:32px 24px 40px}
.panel{background:var(--card);border:1px solid var(--card-br);border-radius:18px;padding:30px}
.panel h1{font-size:clamp(22px,4vw,30px);font-weight:900;margin-bottom:18px}
.panel h1::before{content:"🐋 ";font-size:20px}
.panel p{color:var(--tx2);font-size:15px;margin-bottom:12px}
.panel p b{color:var(--tx)}
.panel code{background:var(--code);border-radius:6px;padding:1px 6px;font-size:13px;color:var(--ac)}
footer{border-top:1px solid var(--card-br);margin-top:36px}
footer .in{max-width:1200px;margin:0 auto;padding:26px 24px;color:var(--tx2);font-size:13px;display:flex;gap:16px;flex-wrap:wrap;justify-content:space-between}
</style>
</head>
...
<footer><div class="in"><div>__FOOTER_L__</div><div>__FOOTER_R__</div></div></footer>
<script>
function toggleTheme(){
  const h=document.documentElement,t=h.dataset.theme==='dark'?'light':'dark';
  h.dataset.theme=t;localStorage.setItem('dsh-theme',t);
  document.getElementById('themeBtn').textContent=t==='dark'?'__BTN_LIGHT__':'__BTN_DARK__';
}
(function(){const t=localStorage.getItem('dsh-theme');if(t){document.documentElement.dataset.theme=t;document.getElementById('themeBtn').textContent=t==='dark'?'__BTN_LIGHT__':'__BTN_DARK__';}})();
</script>
</body>
...
def render_content(kind, lang_key):
    L = S[lang_key]
    other = "en" if lang_key == "zh" else "zh"
    h2 = L["about_h2"] if kind == "about" else L["privacy_h2"]
    title = L["about_title"] if kind == "about" else L["privacy_title"]
    paras = L["about_body"] if kind == "about" else L["privacy_body"]
    body = "".join(f"<p>{esc(t.format(total=total))}</p>" for t in paras)
    home = "/en.html" if lang_key == "en" else "/"
    page = CONTENT_TPL
    reps = {
        "__HTML_LANG__": L["html_lang"],
        "__TITLE__": f"{title} · DSH Plugins",
        "__META_DESC__": title,
        "__CANON__": SITE + (f"en/{kind}.html" if lang_key == "en" else f"{kind}.html"),
        "__ALT_ZH__": SITE + f"{kind}.html", "__ALT_EN__": SITE + f"en/{kind}.html",
        "__HOME__": home, "__BACK__": L["d_back"].replace("← ", ""),
        "__LANG_HREF__": f"/{'en/' if other == 'en' else ''}{kind}.html",
        "__LANG_HREFLANG__": "zh-CN" if other == "zh" else "en",
        "__NAV_LANG__": "中文" if lang_key == "en" else "English",
        "__BTN_LIGHT__": L["btn_to_light"], "__BTN_DARK__": L["btn_to_dark"],
        "__H2__": esc(h2), "__BODY__": body,
        "__FOOTER_L__": L["footer_l"], "__FOOTER_R__": L["footer_r"],
    }
    for k, v in reps.items():
        page = page.replace(k, v)
...
# ---------------- build all pages ----------------
zh = build("zh", "/en.html", "en")
en = build("en", "/", "zh-CN")
write_rel("index.html", zh)
write_rel("en.html", en)
print("index.html:", f"{len(zh)/1024:.0f} KB", "| en.html:", f"{len(en)/1024:.0f} KB")

count = 0
for p in plugins:
    write_rel(detail_rel(p, "zh").lstrip("/"), render_detail(p, "zh"))
    write_rel(detail_rel(p, "en").lstrip("/"), render_detail(p, "en"))
    count += 2
print("detail pages:", count)

for kind in ("about", "privacy"):
    write_rel(f"{kind}.html", render_content(kind, "zh"))
    write_rel(f"en/{kind}.html", render_content(kind, "en"))
print("about/privacy pages: 4")
...
write_rel("manifest.webmanifest", json.dumps(manifest, ensure_ascii=False, indent=2))

# feed.xml (RSS, top 24 by stars, links to on-site detail pages)
items = []
for p in plugins[:24]:
    items.append(f"""    <item>
      <title>{esc(p['n'])} ★ {p['s']}</title>
      <link>{detail_url(p, 'zh')}</link>
      <guid isPermaLink="true">{detail_url(p, 'zh')}</guid>
...
"""
write_rel("feed.xml", feed)

# sitemap.xml (home + about/privacy + every plugin detail, zh & en, with hreflang alternates)
def _url_entry(loc, alt_zh, alt_en, priority):
    return f"""  <url>
    <loc>{loc}</loc>
    <xhtml:link rel="alternate" hreflang="zh-CN" href="{alt_zh}"/>
...
entries = [
    _url_entry(SITE, SITE, SITE + "en.html", "1.0"),
    _url_entry(SITE + "en.html", SITE, SITE + "en.html", "0.9"),
    _url_entry(SITE + "about.html", SITE + "about.html", SITE + "en/about.html", "0.5"),
    _url_entry(SITE + "en/about.html", SITE + "about.html", SITE + "en/about.html", "0.5"),
    _url_entry(SITE + "privacy.html", SITE + "privacy.html", SITE + "en/privacy.html", "0.3"),
    _url_entry(SITE + "en/privacy.html", SITE + "privacy.html", SITE + "en/privacy.html", "0.3"),
]
for p in plugins:
    entries.append(_url_entry(detail_url(p, "zh"), detail_url(p, "zh"), detail_url(p, "en"), "0.7"))
    entries.append(_url_entry(detail_url(p, "en"), detail_url(p, "zh"), detail_url(p, "en"), "0.7"))
sitemap = f"""<?xml version="1.0" encoding="UTF-8"?>
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:xhtml="http://www.w3.org/1999/xhtml">
{chr(10).join(entries)}
</urlset>
"""
write_rel("sitemap.xml", sitemap)
print("favicon.svg / manifest.webmanifest / feed.xml / sitemap.xml written")

# robots.txt
_robots = f"User-agent: *\nAllow: /\n\nSitemap: {SITE}sitemap.xml\n"
write_rel("robots.txt", _robots)
...
if os.path.exists(_llms_src):
    ...
    write_rel("llms.txt", _llms, bom=True)
    print("llms.txt written (with BOM, count=%d)" % len(plugins))

# llms-full.txt — full plugin list with on-site detail links
_lang = S["en"]
_intro = (
    "# DSH Plugins 插件目录（DeepSeek Harness Plugin Directory）\n\n"
    f"> {SITE}\n\n"
    "本站是 DeepSeek Harness（DSH）插件生态的中英双语目录，收录 %d 个社区插件。每个插件都有站内详情页（中英双语），"
    "包含介绍、星标、语言、许可证、版本、最近更新时间与安装命令。数据汇总自各插件上游公开 GitHub 仓库。\n\n"
    "## 全部插件 / All plugins\n\n" % len(plugins)
)
_lines = []
for p in plugins:
